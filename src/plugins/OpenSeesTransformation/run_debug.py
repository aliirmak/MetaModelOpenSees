...
from OpenSeesTransformation import OpenSeesTransformation

logger = logging.getLogger('OpenSeesTransformation')

# Modify these or add option or parse from sys.argv (as in done in run_plugin.py)
PORT = '5555'
PROJECT_NAME = 'OpenSees'
BRANCH_NAME = 'master'
ACTIVE_NODE_PATH = '/Es'
ACTIVE_SELECTION_PATHS = []
NAMESPACE = ''
METADATA_PATH = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'metadata.json')
TEST_CONFIG_PATH = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'test_config.json')
...
node_process = subprocess.Popen(['node', COREZMQ_SERVER_FILE,
                                 PROJECT_NAME,
                                 '-p', PORT,
                                 '-m', METADATA_PATH,
                                 '-j', TEST_CONFIG_PATH],
...
plugin = OpenSeesTransformation(webgme, commit_hash, BRANCH_NAME, ACTIVE_NODE_PATH, ACTIVE_SELECTION_PATHS, NAMESPACE)
